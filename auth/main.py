...
def login(form_data: OAuth2PasswordRequestForm = Depends (), db: Session = Depends (get_db)):
    ...
    if not utils.verify_password(form_data.password, user.hashed_pass):
...
def get_current_user(token: str = Depends(oauth2_scheme)):
    credential_exception = HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Counld not validate credentials",
...
@app.get("/profile")
def profile(current_user: dict = Depends(require_roles(["user", "admin", "string"]))):
    return {"message": f"profile of {current_user['username']} ({current_user['role']})"}


@app.get("/user/dashboard")
def user_dashboard(current_user: dict = Depends(require_roles(["string"]))):
